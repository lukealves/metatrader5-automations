...
Este projeto tem como objetivo automatizar o trading e a extração de dados históricos do MetaTrader 5 (MT5), permitindo que traders e analistas financeiros coletem informações do mercado de forma estruturada e eficiente, além da automação de negociações. Utilizando Python e diversas bibliotecas, o código se conecta ao MT5, coleta dados de ativos específicos e oferece a opção de salvar essas informações em arquivos `.csv` ou `.xlsx`.

## Automação de Trading

Em 2023, iniciei o desenvolvimento deste projeto com a intenção de criar uma solução de trading automatizado, baseada em um modelo LSTM. A ideia original era que o sistema operasse de forma contínua, realizando negociações autônomas por meio da análise de dados e tomada de decisões de compra e venda. Originalmente, o sistema foi desenvolvido para uma versão do MetaTrader 5 disponibilizada pela corretora que eu utilizava na época. Agora, para publicação e uso do público geral, ele foi adaptado para uma versão que pode ser baixada diretamente do site oficial da plataforma, cujo link incluirei mais abaixo.

Embora o MetaTrader 5 oferecido pela maioria das corretoras seja gratuito, elas exigem a criação de uma conta de investidor para que ele possa ser utilizado. Já a versão disponível no site da **MetaQuotes** permite acesso ilimitado a dados do mercado de câmbio (`Forex`) em uma conta de teste, sem a necessidade de cadastro em corretoras, tornando o processo de testes do sistema muito mais simples e prático. No meu caso em específico, o MetaTrader 5 era oferecido pela corretora **Rico**, uma das que disponibilizam essa plataforma no Brasil. Os ativos negociados estavam atrelados ao `Ibovespa`.

Atuei de forma autônoma no mercado, mas não de maneira integral, entre os períodos de agosto de 2023 até maio de 2024, com foco no **trading algorítmico** e no entendimento da resolução de problemas como desenvolvedor, sem necessariamente ter a intenção de me tornar um trader. Durante esse período, realizei testes e aprimoramentos no código para alcançar resultados cada vez mais assertivos, alternando diversas vezes entre contas de teste e a conta real. Esse sistema foi fundamental em vários momentos, especialmente em decisões e trades bem-sucedidos, que dificilmente eu teria alcançado sozinho. Embora não seja um trader profissional, sempre agi com cautela e responsabilidade, respeitando meus limites — um conselho que deixo para quem desejar utilizar este código em operações reais.

Posteriormente, para a publicação deste repositório, optei por simplificar a arquitetura, transferindo a integração com o LSTM para um projeto separado, mais focado no aprendizado de máquina. Apesar dessa mudança, as funcionalidades relacionadas às condições e automações do processo de trading foram preservadas no código. Isso garante que elas possam servir como alicerce para um futuro projeto mais robusto, com potencial para integrar interfaces gráficas e aproveitar recursos de processamento paralelo, como o `Celery`.
